...
"""
tactile_routeB_ams_2_tactilefirst.py

Tactile-first modifications (drop-in replacement for your current script):

Key changes:
- Enforce tactile constraints:
  - max textures <= 5 (default 3)
  - minimum clearance (default 3.0mm) applied as erosion on each region
  - more conservative, tactile-safe texture set chosen by REGION RANK (not hue)
  - optional legend/key plate generation (default ON) with >= 13mm samples
- Default outline (line) height set to ~1.0mm (trackable) instead of 3.0mm
- Parameters grouped as tactile constraints

Usage (example):
  python3 tactile_routeB_ams_2_tactilefirst.py input.png \
...
def erode_by_mm(mask_u8: np.ndarray, mm: float, scale_mm_per_px: float) -> np.ndarray:
    """Erode mask by a physical mm amount to enforce clearance between regions."""
    if mm <= 0:
        return mask_u8
    px = int(round(mm / scale_mm_per_px))
    px = max(px, 1)
...
def triangulate_polygon_to_mesh(poly: Polygon, height: float):
    ...
    m = trimesh.creation.extrude_triangulation(v, f, height=height)
    # trimesh 4.11.x compatibility (no remove_duplicate_faces)
    try:
        # merge_vertices exists in 4.11.x and helps reduce duplicates
        m.merge_vertices()
    except Exception:
        pass
...
# -------------------------
# tactile-safe procedural textures
# (rank-based, not hue-based)
# -------------------------
...
def texture_by_rank(geom, rank: int, pitch_base: float, width_base: float):
    """
    Tactile-safe set with intentionally different "feel":
    - rank 0: wide stripes (directional)
    - rank 1: dots (punctate)
    - rank 2+: crosshatch (busy)
    Use pitch/width differences to increase tactile contrast.
    """
    # clamp rank patterns (avoid too many unique textures)
    if rank <= 0:
        return stripes(geom, pitch=pitch_base, width=width_base, angle_deg=0.0), "stripes_0deg"
    elif rank == 1:
        r = max(0.45, 0.45 * width_base)
        return dots(geom, pitch=pitch_base * 1.25, radius=r), "dots"
    else:
        return crosshatch(geom, pitch=pitch_base * 0.95, width=width_base * 0.9, angle_base=0.0), "crosshatch"
...
# Legend / key plate
# -------------------------
def build_legend_plate(
    textures: List[Tuple[str, str]],  # (part_name, texture_name)
    base_thick: float,
    z0: float,
    tex_height: float,
    pitch: float,
    ...
    """
    Build a tactile key plate: N cells, each cell contains a texture sample.
    No text labels (since tactile), but the part STL names correspond to order.
    """
    n = len(textures)
    if n <= 0:
        return None

    plate_w = margin_mm * 2 + n * cell_mm + (n - 1) * gap_mm
    plate_h = margin_mm * 2 + cell_mm

    plate_poly = Polygon([(0, 0), (plate_w, 0), (plate_w, plate_h), (0, plate_h)])
    plate_mesh = triangulate_polygon_to_mesh(plate_poly, base_thick)
    if plate_mesh is None:
        return None
    plate_mesh.apply_translation([0.0, 0.0, z0])

    parts = [plate_mesh]

    for i, (part_name, tex_name) in enumerate(textures):
        x0 = margin_mm + i * (cell_mm + gap_mm)
        y0 = margin_mm
        cell = Polygon([(x0, y0), (x0 + cell_mm, y0), (x0 + cell_mm, y0 + cell_mm), (x0, y0 + cell_mm)])

        # add a small "index notch" bump at top-left corner for ordering
        notch = Polygon([(x0, y0 + cell_mm - 3.0), (x0 + 3.0, y0 + cell_mm - 3.0),
                         (x0 + 3.0, y0 + cell_mm), (x0, y0 + cell_mm)])
        notch_m = triangulate_polygon_to_mesh(notch, 0.8)
        if notch_m is not None:
            notch_m.apply_translation([0.0, 0.0, z0 + base_thick])
            parts.append(notch_m)

        # texture sample: choose by index i (same mapping used in main)
        tex_geom, _ = texture_by_rank(cell, i, pitch_base=pitch, width_base=tex_width)
        if tex_geom is not None and (not tex_geom.is_empty):
            tex_m = extrude_geometry(tex_geom, height=tex_height, z0=z0 + base_thick)
            if tex_m is not None:
                parts.append(tex_m)

        # cell border (trackable outline)
        border = cell.buffer(0.9).difference(cell.buffer(-0.9))
        border_m = extrude_geometry(border, height=1.0, z0=z0 + base_thick)
        if border_m is not None:
            parts.append(border_m)
...
def build_tactile_mesh(
    ...
    legend: bool = True,
    legend_cell_mm: float = 13.0,

    base_thick: float = 2.4,
    line_width_mm: float = 1.0,
    line_height: float = 1.0,  # tactile-first default
    ...
    scale = target_width_mm / float(w_px)  # mm per px

    # enforce safe limits
    n_colors = max(1, int(n_colors))
    max_textures = max(1, int(max_textures))
    n_colors = min(n_colors, max_textures, 5)  # hard cap 5
    ...
    for gi, (g, m) in enumerate(zip(groups, merged_masks)):
        ...
        # tactile-first: enforce clearance by erosion (prevents touching regions)
        m2 = erode_by_mm(m2, mm=min_clearance_mm, scale_mm_per_px=scale)
    ...
    top = cleaned[:n_colors]
    if len(top) == 0:
        raise RuntimeError("No valid regions found after tactile constraints. Try lowering min_clearance_mm / min_area_mm2.")

    # relief height: darker -> higher (kept)
    ...
    for rank, (_, area, center_lab, mask01) in enumerate(top):
        ...
        # optional gap shrink for AMS part separation
        if part_gap_mm > 0:
            try:
        ...
        # tactile-first: texture determined by rank, not hue
        tex_geom, tex_name = texture_by_rank(geom2, rank, pitch_base=pitch, width_base=tex_width)
        m_tex = None
        if tex_geom is not None and (not tex_geom.is_empty):
            m_tex = extrude_geometry(tex_geom, height=tex_height, z0=base_thick + h_relief)

        part_name = f"color{rank+1}"
        legend_textures.append((part_name, tex_name))
    ...
    if line_geom is not None and not line_geom.is_empty:
        ...
        if m_line is not None:
            # small voxel fuse makes edges cleaner
            m_line = fuse_mesh_voxel(m_line, pitch_mm=min(0.25, fuse_pitch))
            parts.append(m_line)
            named_parts["line"] = m_line

    # legend plate (optional)
    if legend and len(legend_textures) > 0:
        leg = build_legend_plate(
            legend_textures,
            base_thick=base_thick,
            z0=0.0,
            tex_height=tex_height,
            pitch=max(pitch, 2.5),
            tex_width=max(tex_width, 0.9),
            cell_mm=max(legend_cell_mm, 13.0),
        )
        if leg is not None:
            # place below the main base (negative Y)
            bbox = base_mesh.bounds if base_mesh is not None else np.array([[0, 0, 0], [w_px * scale, h_px * scale, base_thick]])
            main_w = float(bbox[1][0] - bbox[0][0])
            # center legend under main shape
            leg_bb = leg.bounds
            leg_w = float(leg_bb[1][0] - leg_bb[0][0])
            dx = (main_w - leg_w) * 0.5
            dy = - (max(25.0, 0.18 * target_width_mm))  # space between main and legend
            leg.apply_translation([dx, dy, 0.0])

            if as_parts:
                leg = fuse_mesh_voxel(leg, pitch_mm=fuse_pitch)
                named_parts["legend"] = leg
            parts.append(leg)
    ...
    if debug_dir:
        ...
        cv2.imwrite(os.path.join(debug_dir, "picked_top_regions_after_clearance.png"), vis)
...
def main():
    ...
    ap.add_argument("--n_colors", type=int, default=3, help="auto pick top N regions by area (tactile-first default 3)")
    ap.add_argument("--max_textures", type=int, default=5, help="hard cap textures (<=5 recommended)")

    # tactile constraints
    ap.add_argument("--min_clearance_mm", type=float, default=3.0, help="min tactile clearance between regions (erode each region)")
    ap.add_argument("--legend", action="store_true", help="add a tactile legend/key plate under the graphic")
    ap.add_argument("--no_legend", action="store_true", help="disable legend/key plate")
    ap.add_argument("--legend_cell_mm", type=float, default=13.0, help="legend texture sample cell size (>=13mm recommended)")

    ap.add_argument("--base_thick", type=float, default=2.4)
    ap.add_argument("--line_width_mm", type=float, default=1.0)
    ap.add_argument("--line_height", type=float, default=1.0)
    ...
    legend_on = (args.legend or True) and (not args.no_legend)
    ...
    mesh, named_parts = build_tactile_mesh(
        ...
        legend=legend_on,
        legend_cell_mm=args.legend_cell_mm,
